...
// side panel state
const selected = ref(null) // selected project object
const pc = ref({ category_id: null, base_cost_usd: '' }) // project category form
const bom = ref({ category_id: null, component_id: null, quantity: 1, unit_price_usd: '', note: '' }) // project BOM form
...
const search = () => projects.fetchAll({ q: q.value })
const clientName = (id) => clients.items.find(c => c.id === id)?.name || '—'

const openManage = async (p) => {
  selected.value = p
  await catalog.fetchCategories()
  await projects.openDetail(p.id)
}

const filteredComponents = computed(() => {
  if (!bom.value.category_id) return []
  return catalog.components.filter(x => x.category_id === bom.value.category_id)
})
...
onMounted(async () => {
  if (!clients.items.length) await clients.fetchAll()
  await projects.fetchAll()
})
</script>
...
<template>
  <div class="p-6 space-y-6">
    ...
    <div class="grid md:grid-cols-2 gap-6">
      <!-- Create form -->
      <form @submit.prevent="submit" class="space-y-2 border rounded p-4">
        ...
        <button class="rounded bg-black text-white px-3 py-2">Add project</button>
      ...
      <div>
        ...
        <ul class="divide-y border rounded">
          <li v-for="p in projects.items" :key="p.id" class="p-3 flex items-start justify-between gap-3">
            <div>
              <div class="font-medium">{{ p.code }} — {{ p.name }}</div>
              <div class="text-sm text-gray-600">Client: {{ clientName(p.client_id) }}</div>
            ...
            <div class="flex gap-3">
              <button class="text-sm underline" @click="openManage(p)">Manage</button>
              <button class="text-red-600 text-sm" @click="projects.remove(p.id)">Delete</button>
            </div>
          </li>
    ...
    <!-- Side panel -->
    <div v-if="selected" class="fixed inset-y-0 right-0 w-full md:w-[560px] bg-white shadow-2xl p-6 overflow-y-auto">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-xl font-semibold">Manage: {{ selected.code }} — {{ selected.name }}</h2>
        <button class="text-sm underline" @click="selected = null">Close</button>
      </div>

      <!-- Categories with base cost -->
      <section class="space-y-3 mb-8">
        <h3 class="font-semibold">Categories (base cost)</h3>
        <form @submit.prevent="addCategory" class="flex gap-2 items-center">
          <select v-model.number="pc.category_id" @change="loadCompsForCat(pc.category_id)" class="border rounded p-2">
        ...
        <ul class="divide-y border rounded">
          <li v-for="row in projects.detail.cats" :key="row.id" class="p-3 flex items-center justify-between">
            ...
            <button class="text-red-600 text-sm" @click="removePc(row)">Remove</button>
          </li>
      ...
      <section class="space-y-3 mb-8">
        ...
        <form @submit.prevent="addBom" class="grid grid-cols-1 md:grid-cols-5 gap-2 items-center">
          ...
          <button class="rounded bg-black text-white px-3 py-2 md:col-span-1">Add</button>
        </form>
        ...
        <ul class="divide-y border rounded">
          <li v-for="row in projects.detail.bom" :key="row.id" class="p-3 flex items-center justify-between">
            <div>
              <div class="font-medium">
                {{
                  (catalog.components.find(c => c.id === row.component_id)?.name)
            ...
            <button class="text-red-600 text-sm" @click="removeBom(row)">Remove</button>
          </li>
      ...
      <section class="space-y-3">
        <div class="flex items-center justify-between">
          ...
          <button class="text-sm underline" @click="projects.fetchProjectSummary(selected.id)">Refresh</button>
        </div>
